...
import { addLabelRequest, removeLabelRequest } from '../../store/cards/cardActions';
...
export const AddLabel: React.FC<AddLabelProps> = ({ cardID }) => {
  ...
  const handleSelect = (arg: { labelId: number, cardLabelID?: number }) => {
    if (arg.cardLabelID) {
      dispatch(removeLabelRequest({ cardLabelID: arg.cardLabelID, cardID }))
    }
    else {
      dispatch(addLabelRequest({ data: { cardId: cardID, labelId: arg.labelId } }))
    }
  }
